...
use crate::application::api_sync::EnvioApi;
...
pub async fn preparar(db: &DatabaseConnection) -> Result<(), RepoErro> {
    ...
    for table in ["pedido", "item_pedido", "pagamento_pedido"] {
        tx.execute(Statement::from_string(backend, format!(
            "UPDATE {table} SET sync_uid=({}) WHERE sync_uid IS NULL OR sync_uid=''",
            crate::migration::m008::UUID_V4))).await.map_err(erro)?;
    }
    let rows = tx.query_all(Statement::from_string(backend,
        format!("{SNAPSHOT} WHERE p.sincronizado_em IS NULL ORDER BY p.numero LIMIT 500"))).await.map_err(erro)?;
    for row in rows {
        ...
        let original = tx.query_one(Statement::from_sql_and_values(backend,
            "SELECT corpo,enviada FROM nuvem_api_outbox WHERE chave=?", [key.clone().into()])).await.map_err(erro)?;
        let (key, cancel, body) = if let Some(original) = original {
            ...
            if current != snapshot {
                return Err(RepoErro::Persistencia("Venda enviada foi editada; exige reconciliacao explicita".into()));
            }
            if body["cancelado"] != true || snapshot["cancelado"] == true {
                return Err(RepoErro::Persistencia("Pedido pendente nao corresponde a cancelamento API".into()));
            }
            (format!("{uid}:cancelamento"), true, json!({"pedidoUid":uid}))
        } else {
            let state = tx.query_one(Statement::from_sql_and_values(backend,
                "SELECT estoque_status FROM pedido WHERE sync_uid=?", [uid.clone().into()])).await.map_err(erro)?;
            if state.and_then(|r| r.try_get::<String>("", "estoque_status").ok()).as_deref() != Some("pronta") {
                return Err(RepoErro::Persistencia("Pedido legado pendente exige reconciliacao antes da API".into()));
            }
            (key, false, body)
        };
        tx.execute(Statement::from_sql_and_values(backend,
            ...
            vec![key.into(), uid.into(), (cancel as i64).into(), body.to_string().into()])).await.map_err(erro)?;
    }
...
pub async fn pendentes(db: &DatabaseConnection) -> Result<Vec<EnvioApi>, RepoErro> {
    let rows = db.query_all(Statement::from_string(db.get_database_backend(),
        "SELECT chave,pedido_uid,cancelamento,corpo FROM nuvem_api_outbox
         WHERE enviada=0 ORDER BY rowid LIMIT 500".to_string())).await.map_err(erro)?;
    rows.into_iter().map(|row| {
        let body: String = row.try_get("", "corpo").map_err(erro)?;
        Ok(EnvioApi {
            chave: row.try_get("", "chave").map_err(erro)?,
            uid: row.try_get("", "pedido_uid").map_err(erro)?,
            cancelamento: row.try_get::<i64>("", "cancelamento").map_err(erro)? != 0,
            corpo: serde_json::from_str(&body).map_err(erro)?,
        })
    }).collect()
}

pub async fn confirmar(db: &DatabaseConnection, envio: &EnvioApi, resposta: &Value) -> Result<(), RepoErro> {
    let tx = db.begin().await.map_err(erro)?;
    let backend = tx.get_database_backend();
    let now = chrono::Utc::now().to_rfc3339();
    tx.execute(Statement::from_sql_and_values(backend,
        "UPDATE nuvem_api_outbox SET enviada=1 WHERE chave=?", [envio.chave.clone().into()])).await.map_err(erro)?;
    let row = tx.query_one(Statement::from_sql_and_values(backend,
        format!("{SNAPSHOT} WHERE p.sync_uid=?"), [envio.uid.clone().into()])).await.map_err(erro)?;
    let current: Option<Value> = row.map(|r| {
        let text: String = r.try_get("", "corpo").map_err(erro)?;
        serde_json::from_str(&text).map_err(erro)
    }).transpose()?;
    let matches = if envio.cancelamento { current.as_ref().is_some_and(|v| v["cancelado"] == true) }
        else { current.as_ref() == Some(&envio.corpo) };
    if matches {
        let state = resposta.get("estoqueStatus").and_then(Value::as_str);
        if let Some(state) = state {
            tx.execute(Statement::from_sql_and_values(backend,
                "UPDATE pedido SET estoque_status=? WHERE sync_uid=?", [state.into(), envio.uid.clone().into()])).await.map_err(erro)?;
        }
        tx.execute(Statement::from_sql_and_values(backend,
            "UPDATE pedido SET sincronizado_em=? WHERE sync_uid=?", [now.clone().into(), envio.uid.clone().into()])).await.map_err(erro)?;

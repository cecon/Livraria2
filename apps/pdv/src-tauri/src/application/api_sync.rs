...
pub struct EnvioApi {
    pub chave: String,
    pub uid: String,
    pub operacao: OperacaoApi,
    pub cancelamento: bool,
    pub corpo: Value,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum OperacaoApi {
    Venda,
    Cancelamento,
    TurnoAbertura,
    TurnoFechamento,
...
pub fn validar_pagina(pagina: &PaginaApi, inicio: &str) -> Result<(), RepoErro> {
    ...
    for event in &pagina.alteracoes {
        ...
        if n <= last || uuid::Uuid::parse_str(&event.produto_uid).is_err() {
            return Err(erro());
        }
        match (event.operacao.as_str(), &event.produto) {
            ...
            ("upsert", Some(p))
                if p.preco_centavos >= 0
                    && p.preco_centavos <= 9_007_199_254_740_991
                    && !p.codigo.is_empty()
                    && !p.titulo.is_empty()
                    && p.ativo => {}
            _ => return Err(erro()),
    ...
    if pagina.proximo_cursor != last.to_string()
        || (pagina.tem_mais && pagina.alteracoes.is_empty())
    {
        return Err(erro());
    }
    Ok(())
...
pub async fn sincronizar_api(
    nuvem: &dyn NuvemApi,
    local: &dyn ReplicaApi,
    ...
    for envio in local.envios().await? {
        ...
        let ok = match envio.operacao {
            OperacaoApi::Venda => {
                resposta.get("pedidoUid").and_then(Value::as_str) == Some(&envio.uid)
                    && resposta.get("recebido") == Some(&Value::Bool(true))
            }
            OperacaoApi::Cancelamento => {
                resposta.get("pedidoUid").and_then(Value::as_str) == Some(&envio.uid)
                    && resposta.get("cancelado") == Some(&Value::Bool(true))
            }
            OperacaoApi::TurnoAbertura => {
                resposta.get("turnoUid").and_then(Value::as_str) == Some(&envio.uid)
                    && resposta.get("recebido") == Some(&Value::Bool(true))
            }
            OperacaoApi::TurnoFechamento => {
                resposta.get("turnoUid").and_then(Value::as_str) == Some(&envio.uid)
                    && resposta.get("encerrado") == Some(&Value::Bool(true))
            }
            OperacaoApi::CaixaMovimento => {
                resposta.get("movimentoUid").and_then(Value::as_str) == Some(&envio.uid)
                    && resposta.get("recebido") == Some(&Value::Bool(true))
            }
        };
        if !ok {

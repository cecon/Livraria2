...
async fn fixture() -> SeaApiReplica {
    ...
    for s in statements { db.execute(Statement::from_string(db.get_database_backend(), s.to_string())).await.unwrap(); }
    SeaApiReplica { db }
}
...
async fn offline_repeticao_preserva_payload_uids_e_preco_da_venda() {
    ...
    local.confirmar_envio(&second, &json!({"estoqueStatus":"incorporada"})).await.unwrap();
...
async fn cancelamento_durante_envio_preserva_pendente_e_cria_operacao_separada() {
    ...
    local.db.execute(Statement::from_string(local.db.get_database_backend(),
        "UPDATE pedido SET cancelado=1 WHERE numero=6535".to_string())).await.unwrap();
    local.confirmar_envio(&sale, &json!({"estoqueStatus":"incorporada"})).await.unwrap();
    local.preparar_envios().await.unwrap();
    let cancel = local.envios().await.unwrap().remove(0);
    assert!(cancel.cancelamento);
    assert_eq!(cancel.uid, sale.uid);
    local.confirmar_envio(&cancel, &json!({"cancelado":true})).await.unwrap();
    local.preparar_envios().await.unwrap();
    assert!(local.envios().await.unwrap().is_empty());
}
...
async fn alteracao_financeira_apos_recebimento_exige_reconciliacao() {
    ...
    local.confirmar_envio(&sale, &json!({"estoqueStatus":"incorporada"})).await.unwrap();
    local.db.execute(Statement::from_string(local.db.get_database_backend(),
        "UPDATE pedido SET total_centavos=6000,sincronizado_em=NULL WHERE numero=6535".to_string())).await.unwrap();

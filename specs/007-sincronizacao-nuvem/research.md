...
- **Mecânica da dedup**: no merge, antes de inserir um `livro`/`fornecedor` novo, casa pela chave natural (livro=`codigo_barras`; fornecedor=`nome_norm`, índice único já existente na 003, `documento` como desempate); se já existe, trata como update (LWW) do registro existente em vez de criar outro `sync_uid`.
...
- **Cuidado**: os cadastros de 005/006 são principalmente **PDV→nuvem (um dono)**; livro, fornecedor e operador são bidirecionais-mutáveis (D7/D15).

## D15 — Identidade de usuários: dois espaços + operador replicado (senha local)

- **Decisão**: manter **dois espaços de identidade distintos** — (a) **usuário da retaguarda** autentica na nuvem via **Supabase Auth**; (b) **operador do PDV** autentica **localmente** (tabela `usuario`, senha no PDV). O **cadastro de operadores sincroniza bidirecional** (dedup por `usuario`, LWW), mas **apenas a identidade** (`usuario`, `nome`, ativo) — **`senha_hash` NUNCA sai do PDV**. Operador criado no escritório chega com senha vazia = "pendente"; o PDV **exige definir a senha localmente** antes do primeiro login. Além disso, a **venda passa a registrar o operador** (`pedido.operador`), sincronizado para os relatórios do escritório. (Clarificação 2026-07-20.)
- **Rationale**: retaguarda e balcão são pessoas/papéis diferentes; unificar reescreveria o login atual (YAGNI) e arriscaria expor credenciais. Replicar só a identidade dá atribuição e administração remota sem tocar na autenticação offline do PDV nem transportar hash de senha.
- **Alternativas rejeitadas**: unificar num só cadastro de usuários (maior mudança, reescreve login — rejeitado na clarificação); sincronizar `senha_hash` (expõe credencial no app web — proibido); só replicar operadores PDV→nuvem sem o escritório criar (rejeitado: o escritório quer cadastrar operador); vendas sem atribuição (rejeitado: escritório quer "vendido por").
- **Nota de implementação**: `senha_hash` fica **fora do payload** de sincronização; o PDV trata `senha_hash` vazio como "definir senha no primeiro uso". `pedido.operador` é nullable (vendas históricas = operador desconhecido).

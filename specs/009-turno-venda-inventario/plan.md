...
3. **Persistência idempotente nas duas pontas** — o turno persiste por migração **`m009`** (SQLite local) e **`0006_turno.sql`** (nuvem; `0004`/`0005` já usadas pelo #15). Ambas idempotentes.
...
| **IV. Migrations idempotentes por comando** | ✅ PASS | `m009` (SQLite) e `0006_turno.sql` (nuvem), ambas **idempotentes** (re-aplicar não duplica); FK por `sync_uid`; pedidos legados toleram `turno_uid` nulo. Sem recompute server-side novo. |
| **V. Guardrails, Skills & ADRs** | ⚠️ AÇÃO | **ADR-0021 já registra o turno**. Ações: (a) resolver a **colisão de numeração ADR-0019** (o WASM da 008 e a identidade do #15 ambos usaram 0019) renumerando o meu para **ADR-0022**; (b) **corrigir ADR-0021** onde cita `0004_turno.sql` → **`0006_turno.sql`** (0004/0005 foram consumidas pelo #15). Nenhuma decisão arquitetural nova além do turno já decidido. |
...
**Resultado do gate**: **PASS**, condicionado a: (a) renumerar meu ADR do WASM para **0022** (resolver colisão 0019); (b) corrigir a referência de migração da nuvem em ADR-0021 para **`0006_turno.sql`**; (c) respeitar ≤300 linhas nas telas novas (decompor); (d) manter `m009`/`0006_turno.sql` idempotentes. Nenhuma violação exige *Complexity Tracking* nova — a complexidade de WASM/workspace já foi paga e justificada na 008.
...
└── apps/escritorio/
    ...
    └── apps/nuvem/migrations/0006_turno.sql   # NOVO (nuvem): mirror turno_operacao + colunas em pedido + RLS
```
...
- **Fase 1 (data-model.md, contracts/, quickstart.md)**: `TurnoOperacao` (campos, estados, funções puras) + esquema `m009`/`0006_turno.sql` (mirror por `sync_uid`); contratos de porta que o adapter web implementa (abrir/vender/encerrar, inventário); contrato da API WASM do turno; mapa de paridade das telas novas; guia de validação. *(gerado nesta execução)*
...
- **ADR-0021 (correção)** — trocar a referência de migração da nuvem de `0004_turno.sql` para **`0006_turno.sql`** (0004/0005 já usadas pelo #15); manter o restante (entidade, numeração por turno, ciclo, convergência).
